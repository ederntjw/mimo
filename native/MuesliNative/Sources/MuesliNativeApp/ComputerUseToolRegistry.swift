...
enum ComputerUseToolRegistry {
    ...
    static let definitions: [ComputerUseToolDefinition] = [
        ...
        definition(.moveCursor, "Move the visible \(AppIdentity.displayName) CUA cursor to a screenshot coordinate without clicking. Use this before uncertain coordinate clicks to show intent.", required: ["screenshot_id", "x", "y"], properties: [
        ...
        definition(.typeText, "Type literal text using keyboard input. Prefer app_name/app_bundle_id and, when available, element_index/element_id so \(AppIdentity.displayName) can activate the app and focus the editable target before typing.", required: ["text"], properties: [
