...
struct ChatGPTAuthTests {
    ...
    @MainActor
    private func makeAuth() -> ChatGPTAuthManager {
        // Parser/PKCE tests need no token files, directories, or keychain access.
        let tokenFileURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("muesli-chatgpt-oauth-test-\(UUID().uuidString)", isDirectory: true)
            .appendingPathComponent("chatgpt-auth.json")
        return ChatGPTAuthManager(tokenFileURL: tokenFileURL, migrateLegacyKeychain: false)
    }
